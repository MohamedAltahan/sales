                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-5">
                                ...
                                <select name="payment_type" id="payment_type" class="form-control">
                                    <option value="1" @selected(old('payment_type') == 1)> الدفع كاش</option>
                                    <option value="2" @selected(old('payment_type') == 2)>الدفع اجل </option>
                                </select>
                                @error('payment_type')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                ...
                                <select name='store_id' id='store_id' class="form-control ">
                                    ...
                                    @if (@isset($stores) && !@empty($stores))
                                        @foreach ($stores as $info)
                                            {{-- if error happen when you add new secondary treasuriy keep the chosen value --}}
                                            <option @selected(old('store_id') == $info->id) value="{{ $info->id }}">
                                                {{ $info->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
